...
<script>
   ...
   $(document).ready(function(){
    ...
    function dynamic_field(number)
    {
     ...
     html = '<tr>';
           html += '<th style="width:200px;"><select class="form-control select2" onchange="getBalance(this);" name="deposit_from[]"><option selected value="">-No Sourch-</option><option value="cash">Cash</option>@foreach($bankAccount as $account)<option value="{{$account->bank->sort}}-{{$account->acc_no}}">{{$account->bank->sort}}-{{$account->acc_no}}</option>@endforeach</select></th>';
